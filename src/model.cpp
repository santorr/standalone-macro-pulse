...
bool parseKey(const std::wstring& text, uint16_t& key, uint8_t& modifiers) {
    ...
    if (t.size() == 1 && ((t[0] >= L'A' && t[0] <= L'Z') || (t[0] >= L'0' && t[0] <= L'9'))) vk = static_cast<uint16_t>(t[0]);
        ...
        const std::pair<const wchar_t*, int> keys[] = {
            ...
            {L"BROWSERFORWARD", VK_BROWSER_FORWARD},
            {L"BROWSERREFRESH", VK_BROWSER_REFRESH},
            {L"BROWSERSTOP", VK_BROWSER_STOP},
            {L"BROWSERSEARCH", VK_BROWSER_SEARCH},
            {L"BROWSERFAVORITES", VK_BROWSER_FAVORITES},
            {L"BROWSERHOME", VK_BROWSER_HOME},
            {L"VOLUMEMUTE", VK_VOLUME_MUTE},
            {L"VOLUMEDOWN", VK_VOLUME_DOWN},
            {L"VOLUMEUP", VK_VOLUME_UP},
            {L"MEDIANEXT", VK_MEDIA_NEXT_TRACK},
            {L"MEDIAPREVIOUS", VK_MEDIA_PREV_TRACK},
            {L"MEDIASTOP", VK_MEDIA_STOP},
            {L"MEDIAPLAYPAUSE", VK_MEDIA_PLAY_PAUSE},
            {L"LAUNCHMAIL", VK_LAUNCH_MAIL},
            {L"LAUNCHMEDIA", VK_LAUNCH_MEDIA_SELECT},
            {L"LAUNCHAPP1", VK_LAUNCH_APP1},
            {L"LAUNCHAPP2", VK_LAUNCH_APP2},
            {L"MENU", VK_APPS},
            {L"HELP", VK_HELP},
            {L"OEM8", VK_OEM_8},
            {L"NUMSEPARATOR", VK_SEPARATOR},
            {L"CLEAR", VK_CLEAR},
...
std::wstring keyName(uint16_t key, uint8_t modifiers) {
    ...
    const std::pair<int, const wchar_t*> names[] = {
        ...
        {VK_VOLUME_MUTE,L"VolumeMute"},
        {VK_VOLUME_DOWN,L"VolumeDown"},
        {VK_VOLUME_UP,L"VolumeUp"},
        {VK_MEDIA_NEXT_TRACK,L"MediaNext"},
        {VK_MEDIA_PREV_TRACK,L"MediaPrevious"},
        {VK_MEDIA_STOP,L"MediaStop"},
        {VK_MEDIA_PLAY_PAUSE,L"MediaPlayPause"},
        {VK_LAUNCH_MAIL,L"LaunchMail"},
        {VK_LAUNCH_MEDIA_SELECT,L"LaunchMedia"},
        {VK_LAUNCH_APP1,L"LaunchApp1"},
        {VK_LAUNCH_APP2,L"LaunchApp2"},
        {VK_APPS,L"Menu"},
        {VK_HELP,L"Help"},
        {VK_OEM_8,L"OEM8"},
        {VK_SEPARATOR,L"NumSeparator"},
        {VK_CLEAR,L"Clear"},
