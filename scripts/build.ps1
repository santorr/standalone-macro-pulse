...
function Invoke-BuildTool([string]$Tool, [string[]]$ToolArgs) {
    ...
    $process.WaitForExit()
    if ($process.ExitCode) { throw "Échec de $Tool (code $($process.ExitCode))." }
}
...
if ($cmakeCommand) {
    ...
    if (!(Test-Path -LiteralPath $vswhere)) { throw 'Installez Visual Studio 2022 avec le développement Desktop en C++ et CMake.' }
    $vsPath = & $vswhere -latest -products '*' -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationPath
    $cmake = Join-Path $vsPath 'Common7\IDE\CommonExtensions\Microsoft\CMake\CMake\bin\cmake.exe'
}
if (!(Test-Path -LiteralPath $cmake)) { throw 'CMake est introuvable. Ajoutez le composant CMake dans Visual Studio Installer.' }
